...
Source --> [mdn web docs\_ ](https://developer.mozilla.org/en-US/docs/Web/HTTP/Overview)
...
- Typical values include:
  ...
  - `400-499`: Client error response. - `404`: Not Found.
...
- `Content-Type` header to specify the media type of content - Common values for the Content-Type header include `application/json`, `application/x-www-form-urlencoded`, `multipart/form-data`, etc.
- `method`: - `'POST'` - `'PUT'` - `'PATCH`
- `body`: Most of the times, when `Content-Type` is JSON we have to 'stringify' some data object: - `body: JSON.stringify(data)`
